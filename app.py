    ...
    for ix, row in df_disembark.iterrows():
        if row['type'] == 'rescue':
            ...
            if row['bracelet_range_or_numbers'] == 'range':
                df_form['bracelet_number_int'] = df_form['bracelet_number'].fillna(0).astype(int)
                df_form = df_form[~df_form['bracelet_number_int'].between(int(row['range_start']), int(row['range_end']), inclusive='both')]
                df_form = df_form.drop(columns=['bracelet_number_int'])
            elif row['bracelet_range_or_numbers'] == 'numbers':
                df_form = df_form[~df_form['bracelet_number'].isin(row['numbers'].split(', '))]
